...
async def _prepare_bridge_mint(request: Request) -> "_BridgeMintReady | _BridgeMintError":
    ...
    mint (master_key set), resolve the litellm identity from the presented key, and derive the envelope
    keys. Returns a ready context or a precise failure value. Running before the exchange is what makes
    every failure here fail closed without consuming the single-use code. Identity-resolution failures
    keep their origin so the mapper statuses each truthfully. The refresh_token grant has its own phase-1
    (:func:`_prepare_bridge_refresh`), which recovers identity from the presented refresh envelope
    instead of a litellm key."""
...
async def _prepare_bridge_refresh(
    ...
    if not isinstance(opened, BridgeRefreshOpened):
        return "invalid_refresh"
    reloaded = await _reload_active_key_by_hash(opened.identity.key_hash)
    if not isinstance(reloaded, _ResolvedKey):
        return _refresh_key_failure_to_mint_error(reloaded)
    return _BridgeRefreshReady(
        ready=_BridgeMintReady(key_hash=opened.identity.key_hash, keys=keys),
...
def _finish_bridge_mint(
    ...
    refresh_envelope = _mint_refresh_envelope_value(identity, token_response, ready.keys, now, mcp_server)
    body = {
...
async def exchange_token_with_server(
    ...
    # Phase 1 for a bridge oauth_delegate server: validate all preconditions and resolve identity+keys
    # BEFORE the exchange below consumes the single-use code or the upstream refresh token. On a refresh
    # grant the client presents a refresh envelope, so this also unwraps it to the real upstream refresh
    # token the exchange must send; the envelope itself is never forwarded upstream. A failure returns
    # without ever touching the upstream credential. authorization_code and refresh_token have separate
    # phase-1 functions because they recover identity from different places (the presented key vs the
    # sealed refresh envelope).
    bridge_mint_ready: _BridgeMintReady | None = None
    bridge_upstream_refresh: Optional[str] = None
